...
The implementation tries the C++ extension ``_C`` first, falling back
to a pure PyTorch implementation that uses ``torch.linalg.qr`` for
Haar-random orthogonal matrices and ``torch.bucketize`` for quantization.
...
from flashquant.core.codebook import CodebookRegistry, Codebook
...
_USE_C_QUANTIZER = False
try:
    from flashquant import _C  # type: ignore[attr-defined]

    if hasattr(_C, "quantize") and hasattr(_C, "dequantize"):
        _USE_C_QUANTIZER = True
        logger.debug("Using _C quantizer backend")
except ImportError:
    pass

if not _USE_C_QUANTIZER:
    logger.debug("C++ quantizer not available; using pure PyTorch fallback")
...
def haar_orthogonal(dim: int, seed: int = 42) -> torch.Tensor:
    ...
    gaussian = torch.randn(
        dim, dim, generator=gen, device="cpu", dtype=torch.float32
    )
    q, r = torch.linalg.qr(gaussian)
    # Ensure uniform Haar distribution by correcting signs via diagonal of R
...
class TurboQuantMSE:
    ...
    Attributes:
        ...
        codebook: Precomputed Lloyd-Max codebook.
        rotation: Orthogonal rotation matrix, shape (dim, dim).
    ...
    def __init__(self, dim: int, bits: int, *, seed: int = 42) -> None:
        ...
        self.dim = dim
        self.bits = bits
        self.codebook: Codebook = _registry.get(dim, bits)
        self.rotation = haar_orthogonal(dim, seed=seed)
    ...
    def quantize(
        ...
        """
        if _USE_C_QUANTIZER:
            return _C.quantize(  # type: ignore[attr-defined]
                x, self.rotation, self.codebook.boundaries, self.bits
            )
    ...
    def dequantize(
        ...
        """
        if _USE_C_QUANTIZER:
            return _C.dequantize(  # type: ignore[attr-defined]
                indices, norms, self.rotation, self.codebook.centroids
            )
...
class TurboQuantProd:
    ...
    Attributes:
        dim: Vector dimension.
        bits: Total bit budget (bits-1 for MSE, 1 for QJL).
        mse_quantizer: Stage 1 quantizer with (bits-1) bits.
        qjl_dim: Number of QJL projection dimensions.
        qjl_matrix: Random Gaussian projection matrix.

    Examples:
        >>> quantizer = TurboQuantProd(dim=64, bits=4)
        >>> indices, norms, signs, res_norms = quantizer.quantize(
        ...     torch.randn(8, 64)
        ... )
    ...
    def __init__(
        ...
        *,
        ...
        self.qjl_matrix = torch.randn(
            self.qjl_dim, dim, generator=gen, device="cpu", dtype=torch.float32
    ...
    def quantize(
        ...
        qjl_signs = torch.sign(projected).reshape(
            *x.shape[:-1], self.qjl_dim
        )
    ...
    def dequantize(
        ...
        use ``estimate_inner_product`` instead -- it is more accurate because
    ...
    def estimate_inner_product(
        ...
        qjl_correction = (q_projected * qjl_signs).sum(
            dim=-1, keepdim=True
        )
